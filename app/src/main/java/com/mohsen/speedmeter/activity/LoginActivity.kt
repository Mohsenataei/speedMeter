...
import com.mohsen.speedmeter.APP_NAME
...
class LoginActivity : AppCompatActivity() {
    val data = com.mohsen.speedmeter.model.AppInstall()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (com.mohsen.speedmeter.util.SecurePreferences.get(this@LoginActivity, USER) != null) {
    ...
    internal inner class SubAsync(var phone: String, var password: String) :
        ...
        override fun doInBackground(vararg params: String): Int? {
            try {
                com.mohsen.speedmeter.util.SecurePreferences.put(this@LoginActivity, "phone", phone)
                if (com.mohsen.speedmeter.util.Util.isEmpty(password)) { // Check Sub
                    var response =
                        com.mohsen.speedmeter.util.JSONParser().parsGet(LOGIN_ADDRESS + "checkSubOtp.jsp?p=" + phone + "&t=" + SERVICE_CODE)
                    if (response == "1")
                        return 0 // it's OK
                    else { // Send OTP
                        response =
                            com.mohsen.speedmeter.util.JSONParser().parsGet(LOGIN_ADDRESS + "action.jsp?type=generateOtp&channel=" + CHANNEL + "&msisdn=" + phone + "&sc=" + SERVICE_CODE)
                    ...
                    val response =
                        com.mohsen.speedmeter.util.JSONParser().parsGet(LOGIN_ADDRESS + "action.jsp?type=confirmOtp&channel=" + CHANNEL + "&msisdn=" + phone + "&sc=" + SERVICE_CODE + "&otp=" + password)
        ...
        override fun onPostExecute(result: Int?) {
            ...
            dialog!!.dismiss()
            if (result == 0) {
                com.mohsen.speedmeter.util.SecurePreferences.put(this@LoginActivity, USER, phone)
                com.mohsen.speedmeter.util.SecurePreferences.put(this@LoginActivity, "phone", phone)
                DeviceName.with(this@LoginActivity).request { info, error ->

                    data.mobile = com.mohsen.speedmeter.util.SecurePreferences.get(this@LoginActivity, "phone")
                    data.type = "login"
    ...
    private fun checkForUpdate() {
        RetrofitSingleton.getWebService().getResults(
            "Android", "roja",
            com.mohsen.speedmeter.BuildConfig.VERSION_CODE
        )
            .enqueue(object : Callback<com.mohsen.speedmeter.component.update.Repo.model.UpdateResult> {
                override fun onResponse(
                    call: Call<com.mohsen.speedmeter.component.update.Repo.model.UpdateResult>,
                    response: Response<com.mohsen.speedmeter.component.update.Repo.model.UpdateResult>
                ) {
                    val code = response.code()
                    val body = response.body()
                    if (body != null && code > 199 && code < 300)
                        com.mohsen.speedmeter.component.update.UpdateFragment.newInstance(body).show(
    ...
    private fun sendServerUserData() {
        data.userInfo.ip = com.mohsen.speedmeter.util.Util.getIPAddress(true)
        if (data.userInfo.deviceName.isNullOrEmpty()) {
            val deviceCode = com.mohsen.speedmeter.util.Util.getDeviceName()
    ...
    private fun sendAppInstall() {
        ...
        data.androidId = com.mohsen.speedmeter.util.InstallationHelper.getAndroidId(this)
        data.installationId = com.mohsen.speedmeter.util.InstallationHelper.getInstallationId(this)
        data.initUserInfo()

        RetrofitSingleton.getWebService().addAppInstall(data).enqueue(object : Callback<com.mohsen.speedmeter.model.AppInstallResponse> {
